<template>
  <v-app>
    <v-app-bar flat class="border-b" color="white" height="64">
      <v-container class="d-flex align-center fill-height">
        
        <div class="text-h5 font-weight-bold brand-logo" @click="closeMenuAndNavigate('/')">
        ...
        <div class="d-none d-md-flex align-center gap-1">
          ...
          <v-btn color="primary" variant="elevated" rounded="lg" to="/auth/sign-in" class="text-none ml-4">
            Sign In
          </v-btn>
          <v-btn color="secondary" variant="elevated" rounded="lg" to="/auth/sign-in" class="text-none ml-4">
            Sign Up
          </v-btn>
        </div>
    ...
    <v-expand-transition>
      <div v-if="isMenuOpen" class="mobile-menu-overlay d-md-none bg-white">
        <v-container class="d-flex flex-column h-100 pa-6">
          ...
          <div class="mt-auto pt-8 d-flex align-center justify-space-between w-100 px-2">
            <div class="d-flex gap-4">
              <a href="https://facebook.com" target="_blank" class="text-blue-darken-2 social-icon">
                <v-icon size="28">mdi-facebook</v-icon>
              </a>
              <a href="https://instagram.com" target="_blank" class="text-pink-accent-3 social-icon">
...
<style scoped>
  ...
  .custom-menu-btn {
    ...
    border-color: #adadad !important; /* Серый контур как в Figma */
  }
